...
/**
 * Groq Service
 * Manages API interactions with official Groq Chat Completions API.
 */
class GroqService {
  constructor() {
   ...
   */
  getApiKey() {
    return (process.env.GROQ_API_KEY || env.GROQ_API_KEY || '').trim();
  }
   ...
   */
  getModel() {
    return (process.env.GROQ_MODEL || env.GROQ_MODEL || 'llama-3.3-70b-versatile').trim();
  }
  ...
  validateConfig() {
    ...
    if (!apiKey || typeof apiKey !== 'string' || apiKey.trim() === '' || apiKey.includes('your-key') || apiKey.includes('xxxxxxxx')) {
      const error = new Error('GROQ_API_KEY is missing or invalid in environment variables');
    ...
    if (!model || typeof model !== 'string') {
      const error = new Error('GROQ_MODEL must be a non-empty string');
      error.statusCode = httpStatus.INTERNAL_SERVER_ERROR;
      error.code = errorCodes.AI_CONFIG_ERROR;
      throw error;
    }
  ...
  extractJson(responseText) {
    ...
    try {
      ...
      const error = new Error('Failed to parse JSON response from Groq AI');
      error.statusCode = httpStatus.INTERNAL_SERVER_ERROR;
      error.code = errorCodes.AI_PARSING_ERROR;
  ...
  /**
   * Sends a prompt to Groq Chat Completions API.
   * Supports calling as:
  ...
  async sendPrompt(arg1, arg2) {
    ...
    if (arg2 !== undefined && typeof arg2 === 'string') {
      ...
      const err = new Error('Invalid prompt text provided to GroqService');
    ...
    logger.info(
      ...
      'Dispatching prompt request to Groq API'
    );
    ...
    try {
      ...
      if (!response.ok) {
        ...
        logger.error(
          ...
          `Upstream Groq API error response (HTTP ${response.status})`
        );
        
        if (response.status === 401) {
          const err = new Error('Unauthorized: Invalid Groq API key provided');
        ...
        const err = new Error(`Groq API request failed with status ${response.status}: ${errorText}`);
      ...
      if (!text || typeof text !== 'string') {
        const err = new Error('Empty or invalid content returned from Groq AI response');
      ...
      logger.info(
        ...
        'Groq AI prompt completed successfully'
      );
      ...
      if (error.name === 'AbortError') {
        const err = new Error(`Groq AI request timed out after ${this.timeoutMs}ms`);
        err.statusCode = httpStatus.SERVICE_UNAVAILABLE;
        err.code = errorCodes.AI_SERVICE_ERROR;
        throw err;
      }
  ...
  /**
   * Readiness check method for Groq service.
   */
  async testConnection() {
    const result = await this.sendPrompt('You are helpful.', 'Reply with exactly: {"status":"OK"}');
    const returnedText = (result.text || '').trim();

    if (!returnedText.includes('OK')) {
      const error = new Error(`Groq connection test failed: Received "${returnedText}"`);
